...
describe LogStash::Inputs::Gelf do
  ...
  context "when interrupting the plugin with TCP" do
    let(:port) { Flores::Random.integer(1024..65535) }
    let(:host) { "127.0.0.1" }
    let(:chunksize) { 1420 }
    let(:use_tcp) { true }
    let(:producer) { InfiniteGelfProducer.new(host, port, chunksize, {:use_tcp => true}) }
    let(:config) {  { "host" => host, "port" => port, "use_tcp" => use_tcp } }
  ...
  it "should be valid for protocol TCP" do
    port = 12209
    host = "127.0.0.1"
    chunksize = 1420
    use_tcp = true
    gelfclient = GELF::Notifier.new(host, port, chunksize, {:use_tcp => true })
    ...
    events = input(conf_tcp) do |pipeline, queue|
      # send a first message until plugin is up and receives it
      while queue.size <= 0
        gelfclient.notify!("short_message" => "prime")
        sleep(0.1)
